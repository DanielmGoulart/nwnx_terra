...
//	uint32_t 								cre_minimapcount;	/* 0x049C */
	...
	uint32_t							cre_total_weight;					/* 0x0B8C */
  	...
  	uint8_t 							spacer5_091;
  	uint8_t 							spacer5_092;
  	uint32_t 						cre_pm_IsPolymorphed; /* 0xBEC */
	...
	uint32_t							field_C68;
	uint32_t							field_C6C;
	CNWSCreatureAdditionalData		*cre_add_data;		/*  0x0C70*/
	CNWSCreature_s(int a2, unsigned int a3, unsigned int a4);
	~CNWSCreature_s();
	
	void				ApplyOnHitVampiricRegeneration(CNWSObject *obj, CNWSItemProperty *prop);
	...
	signed int			AddAttackActions(nwn_objid_t target, int a3, int a4, int a5);
	int					GetIsPossesedFamiliar();
	void				BroadcastAttackofOpportunity(nwn_objid_t target, int tumble);
